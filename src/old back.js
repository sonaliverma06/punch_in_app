...
router.get("/user/:id", async (req, res) => {
  ...
  await pool.getConnection((err, connection) => {
    ...
    connection.query(
      "SELECT date, GROUP_CONCAT(`punchin`) as 'attendance' FROM `attendance` group by date",
      [ids],
      (error, response) => {
        connection.release();
        console.log("response", response);
...
router.post("/punchatten/", async (req, res) => {
  ...
  await pool.getConnection((err, connection) => {
    ...
    const finddata = `SELECT * FROM attendance WHERE signup_user_id=${data.signup_user_id}`;
    console.log("data", data);

    connection.query(finddata, (error, response) => {
      // connection.query( (error, response) => {
      console.log("error", error);
...
// router.get("/datapunch", async (req, res) => {
//   await pool.getConnection((err, connection) => {
//     if (err) {
//       return res.status(500).json({
//         message: "Fail to connect",
...
//     }
//     connection.query(
//       "SELECT GROUP_CONCAT(`punchin`) FROM `attendance` WHERE `date`",
//       (error, response) => {
//         connection.release();
//         if (error) {
//           return res.status(500).json({
